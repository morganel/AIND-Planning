...
class PgNode():
    ...
    includes instance sets common to both types of
    nodes used in a planning graph
    parents: the set of nodes in the previous level
    children: the set of nodes in the subsequent level
    mutex: the set of sibling nodes that are mutually
    exclusive with this node
    '''
    ...
    def is_mutex(self, other) -> bool:
        ...
        :return: bool
            True if this node and the other
            are marked mutually exclusive (mutex)
        '''
...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        Instance variables calculated:
            literal: expr
                    fluent in its literal form including
                    negative operator if applicable
        Instance variables inherited from PgNode:
            parents: set of nodes connected to this node
            in previous A level; initially empty
            children: set of nodes connected to this node
            in next A level; initially empty
            mutex: set of sibling S-nodes that this node has
            mutual exclusion with; initially empty
        '''
    ...
    def show(self):
        '''helper print for debugging shows literal
        plus counts of parents, children, siblings
...
class PgNode_a(PgNode):
    ...
    def __init__(self, action: Action):
        ...
        Instance variables calculated:
            An A-level will always have an S-level as its parent
            and an S-level as its child.
            The preconditions and effects will become the parents
            and children of the A-level node
            However, when this node is created, it is not yet
            connected to the graph
            prenodes: set of *possible* parent S-nodes
            effnodes: set of *possible* child S-nodes
            is_persistent: bool   True if this is a persistence
            action, i.e. a no-op action
        Instance variables inherited from PgNode:
            parents: set of nodes connected to this node in
            previous S level; initially empty
            children: set of nodes connected to this node in
            next S level; initially empty
            mutex: set of sibling A-nodes that this node has
            mutual exclusion with; initially empty
       '''
        PgNode.__init__(self)
        self.action = action
    ...
    def show(self):
        '''helper print for debugging shows action plus
        counts of parents, children, siblings
    ...
    def precond_s_nodes(self):
        ...
        It is computationally expensive to call this
        function; it is only called by the
    ...
    def effect_s_nodes(self):
        ...
        It is computationally expensive to call
        this function; it is only called by the
    ...
    def __eq__(self, other):
        '''equality test for nodes - compares
        only the action name for equality
...
def mutexify(node1: PgNode, node2: PgNode):
    ''' adds sibling nodes to each other's mutual
    exclusion (mutex) set. These should be sibling nodes!
...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        Instance variable calculated:
            ...
            all_actions: list of the PlanningProblem
            valid ground actions
    ...
    def create_graph(self):
        ''' build a Planning Graph as described in
        Russell-Norvig 3rd Ed 10.3 or 2nd Ed 11.4

        The S0 initial level has been implemented for you.
        It has no parents and includes all of
        the literal fluents that are part of the initial state
        passed to the constructor.  At the start
        of a problem planning search, this will be the same
        as the initial state of the problem.  However,
        the planning graph can be built from any state
        in the Planning Problem

        This function should only be called by the class constructor.

        :return:
            builds the graph by filling s_levels[] and
            a_levels[] lists with node sets for each level
        '''
        ...
        # for each fluent in the initial state,
        # add the correct literal PgNode_s
    ...
    def add_action_level(self, level):
        ...
        :param level: int
            the level number alternates S0, A0, S1, A1, S2, ....
            etc the level number is also used as the
            index for the node set lists self.a_levels[] and self.s_levels[]
        :return:
            adds A nodes to the current level in self.a_levels[level]
        '''
        # TODO add action A level to the planning graph as
        # described in the Russell-Norvig text
        ...
        #   to see if a proposed PgNode_a has prenodes that are
        #   a subset of the previous S level.  Once an
        #   action node is added, it MUST be connected to the S node
    ...
    def add_literal_level(self, level):
        ...
        '''
        # TODO add literal S level to the planning graph
        # as described in the Russell-Norvig text
        # 1. determine what literals to add
        # 2. connect the nodes
        # for example, every A node in the previous level has a list
        # of S nodes in effnodes that represent the effect
        #   produced by the action.  These literals will all be part of
        #   the new S level.  Since we are working with sets, they
        #   may be "added" to the set without fear of duplication.
        #   However, it is important to then correctly create and connect
        #   all of the new S nodes as children of all the A nodes that
        #   could produce them, and likewise add the A nodes to the
    ...
    def update_a_mutex(self, nodeset):
        ...
        if the planning graph is a serial planning graph and the
        pair are nonpersistence actions
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s,
                                   node_s2: PgNode_s):
        '''
        Test a pair of state literals for mutual exclusion, returning True if
